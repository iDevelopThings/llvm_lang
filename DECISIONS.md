...
## 2026-07-27 - Async functions can now declare a result type, read via `result(h)`

Motivating ask: the first item on the coroutines feature's own "explicitly out of scope this round" list (`LANGUAGE.md`) - `sema.TypeCoroutine.Elem` was deliberately kept as a real field, always forced to `TypeVoid`, specifically so this round could fill it in with no representation change.

**Decision: an entry-block alloca as `coro.id`'s own promise argument, read back via `llvm.coro.promise`, one fixed shared alignment constant on both sides** (see `coroPromiseAlign`'s own doc comment, `coroutine.go`, for why a literal constant rather than a real `TargetData` query). `genCoroPrologue` allocates a promise slot (the function's own declared return type) only when non-void, passing its address as `coro.id`'s second argument instead of null - LLVM's coroutine-splitting passes then place it at a fixed offset in the heap frame, exactly as `llvm.org/docs/Coroutines.html` documents. Every `return expr` inside the body stores into this same alloca before its existing final-suspend path. Confirmed directly against a raw, pre-optimization IR dump (not just JIT output): `coro.id(0, %coro.promise, null, null)` and `coro.promise(%handle, i32 8, i1 false)` both carry the same literal `8`, matching LLVM's documented argument order exactly. The new `result(h)` builtin (mirroring `resume`/`done`'s predeclared-`SymFunc` shape) needs no generic type parameter, unlike `AnyAs[T]` - a coroutine handle's own static type already carries its real result type via `TypeCoroutine.Elem`, computed directly at the call site. Reading it before `done(h)` is a real `done(h)`-guarded branch returning the zero value, mirroring `genAnyAsCall`'s own "never a blind load" convention.

**A real, must-fix gap this exposed in existing codegen:** `genFuncCall`'s own struct-return coercion branch keyed only on `entry.retType.Kind == TypeStruct` - harmless while an async function's `retType` was always forced void, but a struct-returning async function's own call site (`h := Coro()`) would now hit it and attempt to bitcast the coroutine *handle* as if it were the struct value. Fixed by excluding `entry.isAsync` from that branch outright (a new `funcEntry.isAsync` field) - an async call's real LLVM result is always the handle, regardless of what its declared `retType` says.

**Status:** shipped. Async + a `yield T` return type (a second, unrelated return-type marker) is now explicitly rejected in `checkFuncDecl`, since removing the old blanket "any declared return type" rejection would otherwise have silently let the two combine. New tests: `src/sema/coroutine_test.go` (declared/wrong-type/missing-return/generator-combo cases, `result(h)` against a void coroutine/non-coroutine/wrong-arg-count), `src/codegen/coroutine_test.go` (int/string/struct results, result-before-done zero value against the same handle, a 3-await coroutine's result read after the last suspend), `src/parser/coroutine_test.go` (return-type parses like an ordinary func's), `src/lsp/coroutines_test.go` extended with a non-void async func. Every pre-existing coroutine test re-run unchanged (`go clean -testcache` then `.\test.ps1`, full suite). `LANGUAGE.md`, `CODEGEN.md`, `docs/current-limitations.md`, `docs/advanced-features.md`, `docs/examples.md`, and `examples/coroutines/coroutines.llx` updated; hands-on verified against the real `llvmc.exe`.

**Independent-review fixes, before merge:** (1) a real crash - `result(h)` after `delete h` read through the freed frame (`genDoneCall` reports a nil handle as done, so `genResultCall` took the load branch against a null promise pointer) - fixed with its own nil-handle check ahead of the `done` check, returning the zero value, matching `resume`/`done`/`delete`'s own existing "safe no-op after delete" convention. (2) A non-void coroutine handle couldn't be stored in a `var`/struct field/function parameter declared as bare `coroutine` at all (`Type.Equal` now distinguishes by `Elem`, and bare `coroutine` always parses to `Elem = void`) - broke `std/scheduler` compatibility outright. Fixed in `checkAssignable`: a bare `coroutine`-typed destination (`Elem == TypeVoid`) accepts a handle with any real `Elem`, since driving one by hand never needs the result type - only `result(h)` does, called on the still-precisely-typed binding. (3) A non-copyable declared result type compiled, silently skipped its own destructor (the frame is only ever `free`d, never destructed), and couldn't even be read out (`result(h)`'s own return value hit an unrelated "cannot copy" diagnostic) - rejected outright at declaration time instead, mirroring the `isAsync && isGenerator` rejection immediately above it. (4) Referencing an async function as a bare value (`f := SomeAsyncFunc`) used to fail LLVM module verification for a void result and, after this round, silently returned garbage for a non-void one (`genFuncThunk` had the identical `retType.Kind == TypeVoid` blind spot `genFuncCall` was fixed for, one function above it) - both now a single clean diagnostic (`typeOfSymbolValue`'s `SymFunc` case, mirroring its own existing variadic-function rejection). Doc corrections: `result(h)`'s own safety claim, the `coroutine` keyword's storage rules, and `docs/current-limitations.md`'s "coroutine-to-coroutine" wording (calling/resuming/reading another coroutine's result from inside an async function already works - only awaiting one directly is out of scope).

## 2026-07-27 - A global type registry: `TypeId`/`TypeIdOf`/`TypeByName`/`AnyNew`/`AnySet`, closing out the `Any`/reflection effort

Motivating ask: the last piece of the `Any`/reflection effort, scoped in a design conversation directly with the user around one principle - keep the compiler-side surface to a handful of primitives, leaving anything higher-level (JSON, an IoC container, ...) to ordinary `.llx` code built on top. Resolves `BLOCKERS.md`'s now-removed "Global type lookup by name" entry: `TypeByName`'s `[]int` return (every match, not one) answers its open "same name, two packages" question directly.

**One shared, flat `id -> descriptor` array, reusing the existing `Any` descriptors** - every primitive and every declared struct/enum (eagerly, in tree/declaration order for determinism) gets a slot; an array/map/pointer shape gets one lazily, on first `TypeId[T]`/`TypeIdOf` reference. `typeDescriptorTy` gained one field, `size` (cached from the same computation `genAnyBox` already does ad hoc), for `AnyNew` to zero-allocate with no static type in hand. The array can't be sized until every lazy id exists, which may be mid-body-codegen - so two small fixed-type globals hold its eventual address, filled in by `finalizeTypeRegistry` once every function body is done (see `src/codegen/typeregistry.go`).

**Two real bugs found and fixed while building the eager whole-program registration pass**, both from asking about a struct/enum sema's own lazy checks never had reason to visit: (1) a struct holding an unboxable field (a function value) crashed `structDescriptor` outright - `isBoxableIntoAny` only ever runs per real `Any`-usage call site, never for a type nobody boxes - fixed by re-deriving the same recursive rule in codegen (`registryAnyBoxable`) and silently excluding such a type from the registry (unreachable via `TypeId`/`TypeIdOf` anyway). (2) `AnyNew` on a perfectly copyable, never-copied-anywhere struct spuriously returned `ok=false` - `sema.IsNonCopyable`'s `Copyable` flag is memoized lazily, only forced by a real copy-check, so an untouched struct read back its Go zero-value (`false`) as "non-copyable" - fixed with a codegen-side `codegenCopyable`, walking `StructInfo.Destructor`/fields directly instead.
