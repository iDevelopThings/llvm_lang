...
**Decision: an entry-block alloca as `coro.id`'s own promise argument, read back via `llvm.coro.promise`, one fixed shared alignment constant on both sides.** `genCoroPrologue` allocates a promise slot (the function's own declared return type) only when non-void, passing its address as `coro.id`'s second argument instead of null - LLVM's coroutine-splitting passes then place it at a fixed offset in the heap frame, exactly as `llvm.org/docs/Coroutines.html` documents. Every `return expr` inside the body stores into this same alloca before its existing final-suspend path. The promise alloca and every `llvm.coro.promise` call reading it back both use one literal constant, `coroPromiseAlign` (8 bytes, safely covering every declared return type this language has), rather than a real `TargetData` query - `src/compiler`'s own `finishPipeline` only pins the module's real DataLayout right before `RunPasses`, after codegen has already finished, so a query at codegen time would see an incomplete layout. Confirmed directly against a raw, pre-optimization IR dump (not just JIT output): `coro.id(0, %coro.promise, null, null)` and `coro.promise(%handle, i32 8, i1 false)` both carry the same literal `8`, matching LLVM's documented argument order exactly. The new `result(h)` builtin (mirroring `resume`/`done`'s predeclared-`SymFunc` shape) needs no generic type parameter, unlike `AnyAs[T]` - a coroutine handle's own static type already carries its real result type via `TypeCoroutine.Elem`, computed directly at the call site. Reading it before `done(h)` is a real `done(h)`-guarded branch returning the zero value, mirroring `genAnyAsCall`'s own "never a blind load" convention, never a possibly-uninitialized read.
