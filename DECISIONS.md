...
**A struct field or array element of enum type needs a second, distinct descriptor flavor** (`enumNestedDescriptor`, one per `*sema.EnumInfo`): `structDescriptor`/`arrayDescriptor` build their own field/element table as a single compile-time constant global, with no runtime value in hand to read a discriminant from, so they can't call `genEnumAnyDescriptor` - `typeDescriptorFor`'s own new `TypeEnum` case returns this variant-agnostic placeholder instead (`fieldCount` 0, name the enum's own type name). `AnyFields` walks the descriptor's own field table, so calling it directly on a nested field's `Any` (without `AnyAs[EnumType]` first) correctly yields nothing rather than risking a type-confused read against whichever variant is genuinely live - the same "nothing to walk" precedent every other field-less kind already uses, chosen over baking in one arbitrary real variant's field table (which would misreport a wrong variant's shape as fact whenever a different one is actually active).
