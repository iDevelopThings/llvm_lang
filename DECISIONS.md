...
**Status:** shipped. See `LANGUAGE.md`'s new "Range loops" section for the full language-level rule and `CODEGEN.md`'s new "Range loops" section for the two lowering strategies. New coverage across `src/parser` (`stmt_test.go` - all three binding shapes, a too-many-bindings recovery case, and an explicit regression proof that the three pre-existing plain `for` forms are unaffected), `src/sema` (`rangefor_test.go` - binding types for both subject kinds, the one-binding wrinkle, break/continue legality, the "never terminates" missing-return interaction, a `range` expression used outside a for-loop header, and the non-copyable-value-binding rejection below), and `src/codegen` (`rangefor_test.go`, JIT-executed - both subject kinds' three binding shapes, break/continue, and destructor unwinding for a body-local declared inside a range-for, both on normal completion and on an early `break`), plus a real worked example (`examples/range/range.llx`) exercised end to end, JIT and AOT alike.

**A genuine gap found during independent verification, fixed before merge:** the first implementation seeded key/value binding types (`checkRangeForStmt`) without ever calling `checkNoIllegalCopy` - every iteration's binding is a real copy out of the map/array's own storage, exactly like `v := m[k]`/`v := arr[i]` already are, but nothing enforced the identical non-copyable rule for a range binding specifically. Ranging over a fixed array of a destructor-owning element type silently compiled and ran, producing one illegal extra destructor call per iteration on a value the language's own copy rule says can never be duplicated - the exact "one layer's check is looser than what a downstream layer can actually handle" failure mode `AGENTS.md`'s review-process section exists to catch, caught here by testing the claim in the first draft's own test comment ("a fixed array of a destructor-owning struct type is legal") against a plain `v := m[k]` of the same element type, which is correctly rejected - proving the two should behave identically and didn't. Fixed by adding `checkNoIllegalCopy` (`allowFresh=false`, matching `return`'s own reasoning: an existing element read out of a container is never a fresh construction) to every real key/value binding. The two `src/codegen` tests that had encoded the buggy premise as their own fixture (ranging over `[3]Resource` with `Resource` itself as the value binding) were rewritten to exercise the same destructor-unwind mechanism legally instead - a `Resource` declared as a body-local each iteration, not the range binding itself - plus a new `src/sema` test (`TestRangeForNonCopyableValueBindingRejected`) proving the rejection directly.
...
**Decision:** the third piece of the "iterators" arc (after Rust-style `match`/`yield` and hardcoded map/array `range`) - a `yield T` return-type marker on a top-level `func` declares a generator function, lowered to a push/callback strategy (the consuming range-for's own body becomes a synthesized callback the generator calls repeatedly), the same shape Go 1.23's own `range-over-func` uses - NOT true suspend/resume coroutines.

**Why push/callback over true coroutines:** already settled in the conversation that scoped this round, not re-litigated here - a real coroutine (a generator function that genuinely suspends mid-body and resumes later, holding its own independent stack/register state) needs either a real stackful-coroutine runtime (a second stack per live generator, context-switch machinery) or CPS-transforming the generator's own body into a state machine at compile time - both substantially bigger undertakings than this round's actual scope, and neither was asked for. A push/callback lowering needs none of that: the generator function's own body simply keeps running on the SAME stack the whole time, calling back into the consumer once per yielded value - the exact mechanical trick `LANGUAGE.md`'s own "Explicitly out of scope" list names directly (no external `.Next()`, no two live generators stepped independently, no mid-function suspend).

**Why `yield T` as a return-type marker over the alternatives considered:** two others were on the table - inferring "this is a generator" purely from a `yield` statement appearing somewhere in the body (no grammar change at all), or a dedicated new `iter` keyword replacing `func` entirely. Body-inference was rejected because a function's own signature should be legible from its declaration alone, without scanning the whole body first - exactly the same "read the signature, not the implementation" principle multi-return `(T1, T2)` return types and this feature's own sibling, match-expression `yield`, both already lean on. A new `iter` keyword was rejected as an unnecessary second way to spell "this is a function" when the existing return-type position already has room for exactly this kind of marker (mirroring `(T1, T2)`'s own precedent of extending the return-type grammar rather than inventing a parallel declaration keyword) - the user explicitly chose the return-type-marker spelling.

**Why the `loopCtx` generalization (`returnFromCallback` mode) is the one genuinely new codegen mechanism, not a parallel break/continue implementation:** every other piece of this feature reuses existing machinery wholesale - the fat-pointer/indirect-call convention `first-class functions`/`lambdas` already established, the identical closure-capture analysis a `FuncLit` already gets (just keyed by a `RangeForStmt` node instead), `genIndirectCall`'s own extraction logic factored into a shared helper. But `break`/`continue` inside the synthesized callback have no real loop to branch within (the generator itself loops, by calling the callback repeatedly) - giving `loopCtx` a discriminated mode (branch to a basic block, XOR return a bool from the callback's own frame) let `genBreakStmt`/`genContinueStmt` stay the ONE shared implementation every loop kind funnels through, rather than forking a second, parallel implementation just for this construct.

**Status:** shipped. See `LANGUAGE.md`'s new "Generator functions" section for the full language-level rule and `CODEGEN.md`'s new "Generator functions" section for the producer/consumer lowering and the `loopCtx` generalization. New coverage across `src/parser` (`generator_test.go` - the `YieldReturnType` grammar shape, and proof that a `FuncLit`/`ExternFuncDecl`'s own return type structurally can't reach it), `src/sema` (`generator_test.go` - the method-receiver restriction, `yield`'s generalized match-expression-or-generator legality, `return`'s restricted legality inside a generator's own body and inside a generator-consuming range-for's own body, the zero/one/two-binding consuming rules, every "generator value used anywhere but a range-for's own subject" rejection, nested generator composition, and capture-analysis reuse), and `src/codegen` (`generator_test.go`, JIT-executed - zero/one-binding consumption, break/continue proved with concrete values distinguishing "stop early" from "skip one", a generator with internal filtering/early-exit logic, and both an outer accumulator and a fresh-per-iteration loop-body local correctly captured), plus a real worked example (`examples/generators/generators.llx`) exercised end to end, JIT and AOT alike.
...
**Decision:** `async func Name(params) { body }` + a bare `await` statement, lowered directly onto LLVM's own first-class coroutine intrinsics (`llvm.coro.id`/`coro.begin`/`coro.suspend`/`coro.resume`/`coro.destroy`/`coro.done`) rather than the push/callback strategy generator functions use - a genuinely different feature, not a generalization of `yield T`. A caller holds the resulting handle (`sema.TypeCoroutine`, a real, storable, non-copyable value) and drives it by hand via `resume(h)`/`done(h)`/`delete h`. See `BLOCKERS.md`'s former "True suspend/resume coroutines" entry (now resolved, removed per that file's own convention) for the investigation that preceded this round.

**Why `async`/`await` as new, distinct keywords rather than extending `yield T`:** the two features have genuinely incompatible lowerings, not two facets of one mechanism. A generator's `yield T` producer never really suspends - it keeps running on the consumer's own call stack the whole time, calling back into a synthesized callback (see this file's own "Generator functions" entry above); a coroutine's `await` performs a REAL suspend (`llvm.coro.suspend`), physically returning control to whichever external call is driving it, with its own independently-allocated heap frame surviving across that boundary. Retrofitting `yield T`'s existing push/callback producer to also support a real suspend point would mean rebuilding it on top of `llvm.coro.*` anyway, at which point there's no shared mechanism left to justify reusing the same keyword - a generator's consuming side (`for v := range Gen(args) {...}`, synthesized-callback capture analysis) has no coroutine equivalent either (a coroutine handle is driven by explicit `resume`/`done` calls, never implicitly by a `range`-for). Two distinct keywords make each feature's own real, different execution model legible from its own declaration, matching this project's established "read the signature, not the implementation" principle (see `yield T`'s own entry).

**Why top-level-only, no closures, this round:** deliberately the smallest useful core primitive, mirroring how `range` shipped and was independently verified before generator functions built on top of it. A coroutine's own frame - which locals are live across which suspend point - is entirely `CoroSplit`'s own problem once the pre-split IR exists; the frontend needs zero capture-analysis work as long as `async func` stays a plain top-level declaration (like `ExternFuncDecl`) rather than a `FuncLit`. Extending this to closures/lambdas, and to one coroutine awaiting another, are natural next rounds explicitly left for later (see `LANGUAGE.md`'s own "Explicitly out of scope" list).

**Why void-only (no declared return type) this round:** reading a finished coroutine's own final result correctly needs `llvm.coro.promise`-based frame storage - a real, bounded LLVM mechanism, but a genuinely separate, easy-to-get-subtly-wrong addition (promise placement/alignment interacting with `CoroSplit`'s own frame layout) on top of an already-large round whose own destructor-correctness matrix was already the highest-stakes verification work in this project so far. Declaring a non-void return type on an `async func` is a clean diagnostic rather than a silent gap - an honestly-scoped deferral, not a half-built feature.

**Why per-`await` dedicated cleanup blocks rather than a saved-suspend-index dispatch:** `llvm.coro.suspend`'s own per-call `switch` already gives each suspend point a distinct case-1 (destroy) target for free - the compile-time-tracked `Generator.destructors` stack (the exact same mechanism every struct/enum destructor already uses) already has everything needed to write each one directly, snapshotted at that exact point. Inventing a saved-index-and-dispatch-later scheme instead would duplicate information LLVM's own switch structure already encodes.

**The real bug, and why it's the one thing worth flagging loudest:** the first working implementation conflated a `coro.suspend` switch's DEFAULT arm with its destroy (case 1) arm - reasonable-looking (only two things can happen: get resumed, or get destroyed), but wrong: [LLVM's own documented shape](https://llvm.org/docs/Coroutines.html#coro-destroy) reserves the default arm for a bare "just suspended, do nothing" outcome (the ramp's own first pass reaching an `await`, or an ordinary resume landing back mid-body), with destroy as its OWN explicit case. Conflating the two makes the ramp's own "haven't been resumed or destroyed yet" sentinel land in the destroy arm, running a live coroutine's cleanup unconditionally on its very first call - confirmed as a real, reproducible double-destruct via JIT execution (`src/codegen`'s own destructor-matrix tests), not caught by IR verification or a build (the module verifies fine and JIT-executes without crashing; it just runs the wrong destructors, silently). This is exactly the failure mode `AGENTS.md`'s review-process section exists to catch, and the reason this round's own verification leaned on JIT-executing every one of the N+1 destroy-point x explicit-delete/scope-exit combinations rather than trusting the IR shape alone.

**Status:** shipped. See `LANGUAGE.md`'s new "Coroutines" section for the language-level rule and `CODEGEN.md`'s for the intrinsic-level lowering. Test coverage spans `src/parser`/`src/sema`/`src/codegen`/`cmd/llvmc`'s own `coroutine_test.go` files (an exhaustive per-suspend-point destructor matrix is `src/codegen`'s own, JIT-executed against the real optimized pipeline since coroutine intrinsics are only lowered there) plus a worked example (`examples/coroutines/coroutines.llx`) run JIT and AOT alike.
