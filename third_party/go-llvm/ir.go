...
// LookupIntrinsicID returns the intrinsic ID matching name (e.g.
// "llvm.coro.begin"), or 0 if name doesn't match any known intrinsic - the
// counterpart of Value.IntrinsicID for declaring an intrinsic rather than
// inspecting one already present in a module.
func LookupIntrinsicID(name string) (id uint) {
	cname := C.CString(name)
	defer C.free(unsafe.Pointer(cname))
...
// IntrinsicIsOverloaded reports whether the intrinsic identified by id (see
// LookupIntrinsicID) is overloaded - e.g. true for "llvm.coro.size" (whose
// real name/signature depends on which of its .i32/.i64 overloads is
// wanted), false for a fixed-signature intrinsic like "llvm.coro.begin". An
// overloaded intrinsic's IntrinsicDeclaration/IntrinsicType calls need
// paramTypes to select an overload; a non-overloaded intrinsic's don't (pass
// nil).
func IntrinsicIsOverloaded(id uint) bool {
	return C.LLVMIntrinsicIsOverloaded(C.unsigned(id)) != 0
}

// IntrinsicDeclaration gets or inserts the declaration of the intrinsic
// identified by id (see LookupIntrinsicID) into m, returning its Value. For
// an overloaded intrinsic (see IntrinsicIsOverloaded), paramTypes must be
// supplied to select which overload - e.g. []Type{ctx.Int64Type()} to get
// "llvm.coro.size.i64" rather than its .i32 overload; pass nil for a
// non-overloaded intrinsic.
func (m Module) IntrinsicDeclaration(id uint, paramTypes []Type) (v Value) {
	pt, ptlen := llvmTypeRefs(paramTypes)
	v.C = C.LLVMGetIntrinsicDeclaration(m.C, C.unsigned(id), pt, ptlen)
	return
}

// IntrinsicType returns the function type of the intrinsic identified by id
// (see LookupIntrinsicID), in c. paramTypes selects an overload exactly as
// Module.IntrinsicDeclaration's own paramTypes does.
func (c Context) IntrinsicType(id uint, paramTypes []Type) (t Type) {
	pt, ptlen := llvmTypeRefs(paramTypes)
	t.C = C.LLVMIntrinsicGetType(c.C, C.unsigned(id), pt, ptlen)
	return
}

// IntrinsicOverloadedName returns the mangled name of the intrinsic
// identified by id (see LookupIntrinsicID) for the overload paramTypes
// selects, exactly as Module.IntrinsicDeclaration's own paramTypes does.
func (m Module) IntrinsicOverloadedName(id uint, paramTypes []Type) string {
	pt, ptlen := llvmTypeRefs(paramTypes)
	var length C.size_t
	cname := C.LLVMIntrinsicCopyOverloadedName2(m.C, C.unsigned(id), pt, ptlen, &length)
	defer C.free(unsafe.Pointer(cname))
